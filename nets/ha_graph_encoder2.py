...
class SkipConnection(nn.Module):
    ...
    def forward(self, input, agent_num=None):
        if agent_num:
            return input + self.module(input, agent_num)
        else:
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, agent_num=4, h=None, mask=None):
        ...
        n_pick = (graph_size - self.agent_num - 1) // 2
        ...
        # pickup -> its delivery
        pick_flat = h[:, self.agent_num+1:n_pick + self.agent_num+1, :].contiguous().view(-1, input_dim)  # [batch_size * n_pick, embed_dim]
        delivery_flat = h[:, n_pick + self.agent_num+1:, :].contiguous().view(-1, input_dim)  # [batch_size * n_pick, embed_dim]
        ...
        V_additional_delivery = torch.cat([  # [n_heads, batch_size, graph_size, key_size]
            torch.zeros(self.n_heads, batch_size, self.agent_num+1, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device),
        ...
        compatibility_additional_delivery = torch.cat([  # [n_heads, batch_size, graph_size, 1]
            -np.inf * torch.ones(self.n_heads, batch_size, self.agent_num+1, dtype=compatibility.dtype, device=compatibility.device),
        ...
        compatibility_additional_allpick = torch.cat([  # [n_heads, batch_size, graph_size, n_pick]
            -np.inf * torch.ones(self.n_heads, batch_size, self.agent_num+1, n_pick, dtype=compatibility.dtype, device=compatibility.device),
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, n_heads, embed_dim, feed_forward_hidden=512, normalization='batch', agent_num=3):
        super(MultiHeadAttentionLayer, self).__init__()
        
        self.attention = SkipConnection(
            MultiHeadAttention(agent_num=agent_num, n_heads=n_heads, input_dim=embed_dim, embed_dim=embed_dim)
        )
        self.normalization = Normalization(embed_dim, 'batch')
    ...
    def forward(self, h, agent_num):
        h = self.attention(h, agent_num)
...
class GraphHAttentionEncoder2(nn.Module):
    def __init__(self, n_heads, embed_dim, n_layers, node_dim=None, normalization='batch', feed_forward_hidden=512, agent_num=5):
        super(GraphHAttentionEncoder2, self).__init__()
        ...
        self.layers = nn.ModuleList([
            MultiHeadAttentionLayer(n_heads=self.n_heads, embed_dim=self.embed_dim, feed_forward_hidden=self.feed_forward_hidden, 
                                    normalization=self.normalization, agent_num=self.agent_num)
            for _ in range(self.n_layers)
        ])

    def forward(self, x, agent_num, mask=None):
